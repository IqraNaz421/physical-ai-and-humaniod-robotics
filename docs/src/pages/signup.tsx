...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('1. Submitting form...'); // DEBUG LOG
    ...
    console.log('2. Validation passed, calling AuthContext signup...'); // DEBUG LOG
    ...
    try {
      ...
      console.log('3. Signup successful!'); // DEBUG LOG

      // Redirect to home on success
      history.push(baseUrl);
    } catch (err) {
      console.error('4. Signup failed in handleSubmit:', err); // DEBUG LOG
      setServerError(err instanceof Error ? err.message : 'Signup failed');
    } finally {
      setLoading(false);
    }
